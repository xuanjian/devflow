...
`setup` 会验证框架配置、安装核心 skill 链接，并检查任务流程依赖：
...
如果不希望脚本改全局 npm 包，也可以先只运行：

```bash
node scripts/install-ai-context.mjs setup
```

再按 `doctor` 提示手动安装 OpenSpec：
...
完整安装说明见 [docs/install.md](docs/install.md)，项目流程说明见 [docs/project-introduction.md](docs/project-introduction.md)。
...
# 新机安装时同时安装 OpenSpec CLI
node scripts/install-ai-context.mjs setup --install-openspec

# 严格检查 ai-context、OpenSpec、superpowers 是否就绪
node scripts/install-ai-context.mjs doctor
