...
- 判断任务等级：L1 / L2 / L3 / L4。
- 判断需要读取哪些 `repos/*.md`、`scenes/*.md`、registry 和项目入口。
...
- 本地 `repos/*.md`、代码事实和 Notion 不一致时，先说明差异，再以当前代码和用户确认结果为准。
...
3. 相关项目：从 `repos/*.md`、`registry/repos.json`、`registry/repo-tools.json`、`registry/notion-sources.json`、Notion 项目地图和当前工作上下文推断。
