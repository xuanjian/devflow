...
第一版页面采用“左侧任务栏 + 中间 PM 聊天 + 右侧状态面板”的工作台：

- 不设独立左侧功能菜单，避免出现不能点击或作用不清的装饰导航。
- 视觉方向采用深色 AI 编排控制台：深色底、细网格、玻璃面板、青蓝/紫色渐变和弱发光边框；但仍按 B2B 工作台处理信息密度，不能做成营销页。
- 左侧固定为更窄的任务栏：显示任务 key、标题、状态、当前阶段，只负责选任务。
- 左侧任务行按状态着色：待启动为中性暗色，执行中为蓝色并用蓝青进度条表示流程进度，待确认为琥珀色，已完成为绿色；所有状态都必须在深色主题下保持可读。
- 新创建且尚未开始的任务固定为 `G0 / Draft`，展示为“待启动”；用户在 PM 聊天里说“开始”“启动项目”等语言后，PM 才推进到启动会/边界探索。
- 任务流程不是固定 G0-G5 全量流水线。PM 在启动会和边界探索后，为当前任务生成专属流程；小 BUG 可以跳过产品/UI和多 Agent，大型跨项目任务才展开完整流程。
- 新建任务入口属于任务栏操作：桌面端在任务栏标题旁放 `+`，点击后弹出新建任务弹窗；右侧任务详情不放新建任务表单。
- 任务可以从任务栏删除；删除前需要确认，删除后同步清理该任务的消息、产物、决策记录和验收反馈。
- 新建任务弹窗中，任务描述使用多行大输入框，并支持上传图片和文件；附件先随任务本地保存，后续再替换成真实上传服务。
- 中间 PM 聊天是主操作区：
  - 用户把需求、纠错、验收反馈、返工意见都发给 PM。
  - `G0 / Draft` 的新任务收到“开始”“启动项目”等启动意图后，由 PM 推进到 `G1 启动会 / 边界探索`。
  - PM 和每个 Agent 都有独立模型配置，配置形态为 `provider + model`；模型名不在前端固定成某一个 GPT 版本，模型下拉选项来自后端 provider/Codex 能力接口。
  - PM 有聊天输入框时，模型切换放在输入框右下角；查看其它 Agent 输出时，因为没有对话输入框，模型切换放在底部“回到 PM”按钮旁边。
  - PM 回复必须展示可审计的分析结果，包括事实、判断、下一步和需要用户补充的信息；等待期间前端显示 PM 正在分析。
  - PM 聊天支持文字、图片和文件。
  - 用户消息靠右展示，PM 回复靠左展示。
...
- 右侧状态面板只负责展示和少量确认动作：
  ...
  - 任务流程；未启动时只显示 `G0 待启动` 和“流程待 PM 启动会后生成”，启动会后显示本任务专属步骤。
...
3. 任务包可生成启动会、项目定义、产品/UI、技术方案等文档；具体产物按本任务流程动态裁剪。
4. 控制台必须展示产物列表，并能过滤待确认产物。
5. 控制台必须支持 PM 聊天、最终验收反馈记录和状态追溯。
6. 技术方案确认前不启动开发。
