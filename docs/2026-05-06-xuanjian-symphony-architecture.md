...
第一版页面采用“窄任务池 + 右侧编排详情”的工作台：

- 左侧导航：任务池、运行 Agent、Workspace、验收与返工、配置。
- 窄任务池：显示 Jira key、标题、状态、组件、当前 Gate，只负责选任务。
- 右侧详情是主操作区：
  - 当前任务怎么操作。
  - Gate 进度。
  - 待确认产物。
...
1. 页面原型已确认：控制台使用“窄任务池 + 右侧编排详情”布局。
